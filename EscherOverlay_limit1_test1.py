...
def draw_line(z1, z2, fill='red', stroke_width=stroke_width):
    line = euclid.shapes.Line(z1.real, z1.imag, z2.real, z2.imag)
    d.draw(line, stroke='red', stroke_width=stroke_width, fill='none')

def drawTiles(drawing, tiles):
    for tile in tiles:
        d.draw(tile, hwidth=0.02, fill='none')
...
draw_line(-1, 1, fill='grey', stroke_width=stroke_width)
draw_line(complex(math.cos(math.pi/2), math.sin(math.pi/2)), complex(math.cos(math.pi+math.pi/2), math.sin(math.pi+math.pi/2)), fill='grey', stroke_width=stroke_width)
draw_line(complex(math.cos(math.pi/3), math.sin(math.pi/3)), complex(math.cos(math.pi + math.pi/3), math.sin(math.pi + math.pi/3)), fill='grey', stroke_width=stroke_width)
draw_line(complex(math.cos(2*(math.pi/3)), math.sin(2*(math.pi/3))), complex(math.cos(math.pi + 2*(math.pi/3)), math.sin(math.pi + 2*(math.pi/3))), fill='grey', stroke_width=stroke_width)
draw_line(complex(math.cos(math.pi/6), math.sin(math.pi/6)), complex(math.cos(math.pi + (math.pi/6)), math.sin(math.pi + (math.pi/6))), fill='grey', stroke_width=stroke_width)
draw_line(complex(math.cos(5*math.pi/6), math.sin(5*math.pi/6)), complex(math.cos(math.pi + (5*math.pi/6)), math.sin(math.pi + (5*math.pi/6))), fill='grey', stroke_width=stroke_width)

d.draw(euclid.shapes.Circle(0, 0, 1), stroke='red', stroke_width=stroke_width, fill='none')

d.setRenderSize(w=400)
d.saveSvg('escherOverlay_limit1_test1.svg')
d.savePng('escherOverlay_limit1_test1.png')