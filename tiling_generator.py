...
p1 = 4
'''Variable for edge number of the first polygon in the tiling (the centre polygon)'''

p2 = 4
'''Variable for edge number of the second polygon in the tiling'''

q = 3
'''Variable for how many of each polygon type meet at each vertex.
Note that the conditions 1/p1 + 1/p2 + 1/q < 1 must hold'''

rotate = 0
'''Variable rotates polygons at each level of the tessellation relative to each other,
recommed 0 for a proper tiling'''
...
d.setRenderSize(w=4000)
d.saveSvg('TilingGenerator.svg')
d.savePng('TilingGenerator.png')